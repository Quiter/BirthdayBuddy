...
class SystemContactDataSource @Inject constructor(
     ...
     */
    private val pureSystemGroups = setOf("starred in android", "my contacts", "all contacts", "contacts")
    ...
    suspend fun updateContactBirthday(contactId: String, birthday: LocalDate): Boolean = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun fetchContactsFromSystem(groups: Map<Long, GroupInfo>): List<Contact> = withContext(Dispatchers.IO) {
        ...
        val birthdaySelection = "${ContactsContract.Data.MIMETYPE} = ? AND ${ContactsContract.CommonDataKinds.Event.TYPE} = ?"
